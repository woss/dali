...
describe('UpdateBuilder', () => {
  it('update by id', async () => {
    ...
    const results = await update(orm, users)
      .id('alice')
  ...
  it('update throws on empty data', async () => {
    await expect(update(orm, users).execute()).rejects.toThrow(
      'Cannot update with empty data',
    );
  });
  ...
  it('update throws on null field name for set', async () => {
    expect(() => update(orm, users).set('', 'value')).toThrow(
